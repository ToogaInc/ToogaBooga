...
export class ShowBlacklist extends BaseCommand {
    public constructor() {
        const cmi: ICommandInfo = {
            ...
            description:
        "Shows a list of all current blacklisted users of the server",
   ...
   */
    public async run(ctx: ICommandContext): Promise<number> {
        const blInfo = ctx
            .guildDoc!.moderation.blacklistedUsers.map(
            (x) =>
                ` Realm Name: ${x.realmName.lowercaseIgn}` + ` - Reason: ${x.reason}`
        )
            .join("\n");

        const usersBf = new StringBuilder()
            .append("================= BlackListed Users =================")
            .appendLine()
            .append(blInfo)
            .appendLine()
            .append("======================= END =========================")
